...
def fitoffset():
    """
    Usage: fitoffset npar_rng npar_azi xcorr.dat [SNR]

            npar_rng    - number of parameters to fit in range 
            npar_azi    - number of parameters to fit in azimuth
    ...
    n = len(sys.argv)  # FIXME: This should take function parameters rather than command line arguments
    ...
    print('FITOFFSET: operating on file ',
          sys.argv[3], ' - Should be xcorr.dat ... ...')
    ...
    k = 0  # TODO: There may be a faster way to do this with numpy
    for i in range(n):
    ...
    def get_final_coeff(c, range_r, range_a): # NOTE: This function does not rely on any global variables. Consiter making it a standalone function.
        shift0 = c[0] - c[1]*(range_r[0]+range_r[1])/(range_r[1]-range_r[0]) - \
            c[2]*(range_a[0]+range_a[1])/(range_a[1]-range_a[0])
    ...
    rshift, sub_int_r, stretch_r, a_stretch_r = get_final_coeff(
        c012, range_r, range_a)
    ashift, sub_int_a, stretch_a, r_stretch_a = get_final_coeff(
        c345, range_r, range_a)
...
# TODO: Determine if these comments are usefull, and remove if not.
...
if __name__ == "__main__":
    _main_func(__doc__)  # FIXME: Allow imports rather than sys.argv for all inputs
