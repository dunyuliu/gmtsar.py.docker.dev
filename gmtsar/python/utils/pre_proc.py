...
def preprocAlosslcAlos2Alosscan(SAT, master, aligned, skip_master, cmdAppendix):
    ...
    if check_file_report('IMG-'+masterCut) is False or \
            check_file_report('IMG-'+alignedCut) is False or \
            check_file_report('LED-'+masterLed) is False or \
            check_file_report('LED-'+alignedLed) is False:
        print(' ')
        print(
            'PREPROC: ERROR - cannot find input file at raw/ for ALOS_SLC/ALOS2/ALOS_SCAN')
...
def fdb2fbs(SAT, master, aligned, prefix=''):
    rng_samp_rate_m = float(grep_value(
        prefix+master+'.PRM', 'rng_samp_rate', 3))
    rng_samp_rate_s = float(grep_value(
        prefix+aligned+'.PRM', 'rng_samp_rate', 3))
    ...
    if t == 1.0:
        masterOrAligned = 0
        print(
            'PREPROC: The range sampling rate for master and aligned are: ', rng_samp_rate_m)
    elif t == 2.0:
        masterOrAligned = 2
        imageName = aligned
    elif t == 0.5:
...
def preprocS1tops(master, aligned, skip_master, ESD_mode):
    ...
    if skip_master == 0:
        run(scriptName+' '+master+' '+master+'.EOF ' +
            aligned+' '+aligned+'.EOF dem.grd '+additionalFlag)
    elif skip_master == 1:
        run(scriptName+' '+master+' 0 '+aligned+' ' +
            aligned+'.EOF dem.grd '+additionalFlag)
    elif skip_master == 2:
        run(scriptName+' '+master+' '+master+'.EOF ' +
            aligned+' 0 dem.grd '+additionalFlag)
    else:
...
def pre_proc():
    ...
    if not (numOfArg == 4 or numOfArg == 6 or numOfArg == 8 or numOfArg == 10 or numOfArg == 12 or numOfArg == 14):
        print(' ')
        print(
            ' Usage: pre_proc SAT master_stem aligned_stem [-near near_range] [-radius RE] [-npatch num_patches] [-fd1 DOPP] [-ESD mode] [-skip_master 1]')
        print(' ')
    ...
    if SAT == 'ALOS':
        ...
        preprocAlosslcAlos2Alosscan(
            SAT, master, aligned, skip_master, cmdAppendix)
    elif SAT == 'CSK_RAW':
