...
"""
# FIXME: filter is a reserved word in python, consider renaming the file
# Most of this file is invoking the same command with different arguments.
# Possibly this can be condensed
...
def filter():
    """
    Usage: filter master.PRM aligned.PRM filter decimation [rng_dec azi_dec] [compute_phase_gradient]

    Apply gaussian filter to amplitude and phase images.
    ...
    if type(rng_samp_rate) is float:
